...
def save_file_csv(name_file, info):
    domain=str(info[0])
    ...
    with open(name_file, mode='a+') as host_csv:
        ...
        hosts.writerow([domain, reverse, hostip ])
...
def lookup_host(start_ip,end_ip,name_file=None,output=False,csv=False):
    ...
    for ip_int in range(int(start_ip), int(end_ip)+1):
        try:
            ...
            strResult = "[+] Host Name for the IP address {} is {}".format(ipaddress.IPv4Address(ip_int), hostNameStr)
            print(strResult)
            if(output==True):
                if(csv==False):
                    save_file(name_file,strResult)
                else:
                    save_file_csv(name_file,hostName)
